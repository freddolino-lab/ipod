...
def compile_idr_results(idr_outfiles,
    ...
    for ctg_id,ctg_info in ctg_array_dict.items():
        ...
        for idx in range(len(ctg_info["frac_passed_array"])):
            ...
            val = ctg_info["frac_passed_array"][idx]
            bg_data.addline(
                ctg_id,
                start,
                end,
...
def get_peaks_from_binary_array(ctg_id, starts, ends, flag_arr, sig_arr, narrowpeak):
    ...
    ctg_id : str
        Name of this contig.
    starts : np.array
        1d numpy array containing start positions for every position analyzed.
    ends : np.array
        1d numpy array containing end positions for every position analyzed.
    flag_arr : np.array
...
def read_grfile(filename, skiprows=0):
    ...
    locs = np.loadtxt(filename, dtype='int', usecols=(0,), skiprows=skiprows)
    vals = np.loadtxt(filename, usecols=(1,), skiprows=skiprows)
    return (locs,vals)
...
def grfiles_to_mat(filenames):
        ...
        locs = np.loadtxt(firstfile, dtype='int', usecols=(0,))
        vals = np.loadtxt(firstfile, usecols=(1,))

        datmat = np.zeros( (len(vals), len(filenames) ) )
        datmat[:,0] = vals
        nvals=len(vals)
...
def remove_nans(infile, outfile,zeros=False):
    ...
    if zeros:
        data[np.invert(goodinds)] = 0.0000
...
def write_grbin(indices, data, filename):
    ...
    np.savez(filename, indices, data)
...
def sort_gr_file(txtfile, outfile):
    ...
    print("Doing zip")
    valpairs = list(zip(offsets, data))
    print("Doing sort")
...
def subtract_percentile_from_genomefile(infile, bgfile, outfile, perc=50):
    ...
    mydat = np.loadtxt(infile, unpack=True, dtype="|S8")
    bgdat = np.loadtxt(bgfile, unpack=True, dtype="|S8")
...
def subtract_percentile_from_pmmm_file(infile, bgfile, outfile, perc=50):
    ...
    mydat = np.loadtxt(infile, unpack=True)
    bgdat = np.loadtxt(bgfile, unpack=True)
...
def calc_ratio(firstfile, secondfile, outfile, assume_common=True,minval=0.0):
    ...
    offsets1, pm1 = np.loadtxt(firstfile, unpack=True)

    offsets2, pm2 = np.loadtxt(secondfile, unpack=True)
...
def gauss_file_ipod(txtfile, outfile, gausswidth=1, width=11, dropvals=0):
    ...
    if (dropvals == 0):
        ...
        while (numdropped < dropvals):
            ...
            usekern = np.array(kernel, copy=False)
            usekern[min_ind] = 0
            usekern[max_ind] = 0
            vallist[min_ind] = np.median(vallist)
            vallist[max_ind] = np.median(vallist)

    curravg = np.dot(vallist, usekern) / np.sum(usekern)
        ...
        if (removeind >= 0):
            ...
            if (newlen == 0):
                ...
                curravg = (
                    ...
                    - np.sum(removedvals) / float(newlen)
                )
                currlen = newlen
...
def convolve_core(offsets, data, gausswidth=4, width=49, genomelength=None, kerneltype="gauss", kernel=None, wrap=True):
    ...
    #  raise("explicit kernel not yet implemented")
        ...
        kernel = np.array([scipy.stats.norm.pdf(x,scale=gausswidth) for x in myxvals])
    ...
    for off_ind,offset in enumerate(offsets):
        ...
        total = np.dot(kernel[offset_slice],data_slice)
        total_weights = np.sum(kernel[offset_slice])
...
def write_ref_mean_std(filenames, outfile, outmeans=False):
    ...
    offsets, vals = np.loadtxt(filenames[0], unpack=True)

    for name in filenames[1:]:
        newvals = np.loadtxt(name, usecols=(1,), unpack=True)
        vals = scipy.vstack((vals,newvals))

    vals = np.ma.masked_array(vals, np.invert(np.isfinite(vals)))
    #print scipy.shape(vals)
    means = np.ma.mean(vals, axis=0)
    stdevs = np.ma.std(vals, axis=0, ddof=1)

    # Fix cases with only one sample in each case

    # Fix cases with no samples
    #sortedvals = np.ma.sort(means)
    #numgood = np.ma.count(sortedvals)
...
def write_ref_log_mean_std(filenames, outfile, outmeans=False):
    ...
    for name in filenames[1:]:
        newvals = np.log2(np.loadtxt(name, usecols=(1,), unpack=True))
        vals = scipy.vstack((vals,newvals))
    ...
    # Fix cases with only one sample in each case
    #goodcounts = np.ma.count(vals, axis=0)
...
def calc_log_foldchange(indat, refdat, outdat):
    ...
    offsets, expvals = np.loadtxt(indat, unpack=True)
    dummy, refvals = np.loadtxt(refdat, unpack=True)
...
def normalize_vs_ref(indat, indat80, refdatmeanstd, refdat80, outraw, outz, oricorr = True, gaps=False):
    ...
    offsets, expvals = np.loadtxt(indat, unpack=True)
    if (oricorr):
        expvals80 = np.loadtxt(indat80, usecols=(1,), unpack=True)
        refvals80 = np.loadtxt(refdat80, usecols=(1,), unpack=True)
    ...
    refmean, refstd = np.loadtxt(refdatmeanstd, usecols=(1,2), unpack=True)
...
def plot_zscore_hist(zscoresfile, histfigfile, flagfile=False, zerolabel="0", onelabel="1", min=-10, max=10, bins=500):
    ...
    indices, data = np.loadtxt(zscoresfile, unpack=True)

    if (flagfile):
        #flagfile_stream, flagfile_use = mkstemp()
        #filter_grfile_by_bpranges(zscoresfile, flagfile, flagfile_use)
        flagdata = np.loadtxt(flagfile, usecols=(1,), unpack=True)
...
def calc_corrcoef(file1, file2, clampvals):
    ...
    data1 = np.loadtxt(file1, usecols=(1,), unpack=True)
    data2 = np.loadtxt(file2, usecols=(1,), unpack=True)
...
#
    ...
    if (startindex > endindex):
        offset_slice = np.concatenate((offsetvec[startindex:],offsetvec[:endindex]))
        data_slice = np.concatenate((datvec[startindex:],datvec[:endindex]))
    else:
        offset_slice = offsetvec[startindex:endindex]
        data_slice = datvec[startindex:endindex]
...
def identify_epods_v2(epod_data, percentile_data, min_epod_length, epod_outfile, lpod=False, delta=25):
    ...
    #np.save('test_centers.npy',epod_pot_arr)
    epod_pot_centers = scipy.signal.argrelextrema(epod_pot_arr, np.greater_equal,mode='wrap')[0]
    epod_abovezero = np.argwhere(epod_pot_arr > -100)
...
def identify_epods_v3(epod_data, percentile_data, min_epod_length, epod_outfile, lpod=False, delta=25):
    ...
    if (lpod):
        print("Searching for regions at least {} bp long with a median z score below {}".format(min_epod_length, lpod_cutoff))
    else:
        print("Searching for regions at least {} bp long with a median z score above {}".format(min_epod_length, epod_cutoff))
...
def identify_epods_v3_opt(epod_data,
                          percentile_data,
                          min_epod_length,
                          epod_outfile,
                          delta=25,
...
def identify_epods_v4(epod_data, percentile_data, min_epod_length, epod_outfile, lpod=False, delta=25):
    ...
    #np.save('test_centers.npy',epod_pot_arr)
    #epod_pot_centers = scipy.signal.argrelextrema(epod_pot_arr, np.greater_equal,mode='wrap')[0]
    epod_centers = np.argwhere(epod_pot_arr > -100)
...
def analyze_tf_occupancy(annfile, ipoddat, tfstr, outfile=None):
    ...
    all_locs = np.loadtxt(annfile, unpack=True, usecols=(3,4), delimiter="\t")

    # load all of the data
    locs, data = np.loadtxt(ipoddat, unpack=True)
...
def get_range_matching_annotations(annotationfile, annotationregexp, seqfile, return_annotations = False):
    ...
    offsets = np.loadtxt(seqfile, unpack=True, usecols=(0,))
...
def find_annotations_at_positions(genomefile, rangelist, seqconvfile, genomelength=4641652, enclosed=True, offsets_bp = False):
    ...
    seqnums = np.loadtxt(seqconvfile, unpack=True, usecols=(0,))
...
def identify_peaks(infile, medfile, minlength, maxlength, percentile, outfile, troughs = False, genlength =4641652 , outfile_startend = None):
    ...
    offsets, datvec = np.loadtxt(infile, unpack=True)
    percdat = np.loadtxt(medfile, usecols=(1,), unpack=True)
...
def calc_epod_rna_stats(epodlocs, rnafile, genomelength, outfile):
    ...
    for start, end in epodlocs:
        ...
        epod_mean_rnas.append(np.mean(epod_rna_scores))
        epod_median_rnas.append(np.median(epod_rna_scores))
        epod_log_median_rnas.append(np.median(scipy.log2(epod_rna_scores)))
...
def calculate_corrcoeffs(file1, file2, name1, name2):
    ...
    mat1 = np.loadtxt(file1_filtered2, unpack=True, usecols=(1,))
    mat2 = np.loadtxt(file2_filtered1, unpack=True, usecols=(1,))
...
def hierclust_arrays(allfiles, allnames, outfile):
    ...
    for file in allfiles:
        allmats.append(np.loadtxt(file, unpack=True, usecols=(1,)))
...
def normalize_by_countfile(infile, countfile, outfile, countscale=10000, offset=0.25):
    ...
    fullcount = float(np.sum(count_data))
...
def normalize_mean_value_withgenome(infile, outfile, targetval=2500.0):
    ...
    genomes, offsets, data = np.loadtxt(infile, unpack=True, dtype="|S8")
...
def gen_flagfile_from_gff(gff, grfile, outfile):
    ...
    all_locs = np.loadtxt(gff, unpack=True, usecols=(3,4),dtype="int", delimiter="\t")
    all_names = np.loadtxt(gff, unpack=True, usecols=(0,), dtype="|S8", delimiter="\t")
...
def get_stats_on_top_percentile(grfile, flagfile, perc=99):
    ...
    flagarr = np.loadtxt(flagfile, dtype='int')
...
def print_zscore_stats_manyflags_forlog(grfile, flagfile, outprefix, sparse=False):
    ...
    for i in range(1,len(flagnames)):
        for k in range(i+1,len(flagnames)):
            ...
            logp_locs = np.array( [0.01*f for f in range(100)] + [0.999, 0.9999, 0.99999,0.999999, 0.9999999])
            ppfs = scipy.stats.norm.ppf( logp_locs )
            logp_vals = -1*np.log10(scipy.stats.norm.sf(ppfs))
...
def quantile_normalize(origmat):
    ...
    sortedmat = np.array(inmat, copy=True)

    nrow, ncol = inmat.shape

    sortedmat.sort(axis=0)
    meanvals = np.mean(sortedmat, axis=1)
...
def score_normed_vals(scorefile, meanstdref, outfile):
    ...
    refinds, means, sigmas = np.loadtxt(meanstdref, unpack=True)
    # Make sure we don't get nans -- replace zeroes with the lowest nonzero value
    #smallestval = np.min(vals[vals > 0.001])
    #vals[vals < smallestval] = smallestval
    zscores = (np.log2(vals) - means) / sigmas
...
def spline_smooth_genome(infile, outfile, plot=True, plotfile="spline.pdf", genome_length = 4641652, oriCloc=3923883, outfile_all = None, perspline=True):
    ...
    sortorder = np.argsort(offs_recentered)

    offs_mod = offs_recentered[sortorder]
    vals_mod = dat[sortorder]
    offs_orig_mod = offs[sortorder]
    resort_order = np.argsort(offs_orig_mod)

    offs_aug = np.append(offs_mod, genome_length+offs_mod[0])
    dat_aug = np.append(vals_mod, vals_mod[0])
    #offs_aug = np.append(offs, genome_length+offs[0])
    #dat_aug = np.append(dat, dat[0])
...
def make_ci_file(allsplinefile, outfile):
    ...
    #all_matchseqs = np.loadtxt(match_seqs_file)
    ...
    write_grfile(probestarts, probe_cis / np.min(probe_cis), outfile)
...
def loess_subtract_counts(file1,file2,outfile,outplot=None,lscalefac=1.0):
    ...
    allinds = list(range(len(locs1)))
    randinds = list(range(0,len(locs1),len(locs1)/500)) + [len(locs1)-1]

    randinds.sort()
    ordered_inds = np.array(randinds)


    sortord = np.argsort(vals2)
    ...
    xmin = np.min(vals2)
    xmax = np.max(vals2)
    xvals_test_plot = np.linspace(xmin,xmax)
    print("done with fit")
    predvals = np.array(growthcurves.do_loess_interpolation_array(list(vals2),'myfit') )
    ...
    pylab.plot(xvals_test_plot,2*np.array(predvals_forplot),'b-')
    pylab.plot(xvals_test_plot,0.5*np.array(predvals_forplot),'r-')
    pylab.plot(xvals_test_plot,lscalefac*np.array(predvals_forplot),'y-')
    pylab.xlabel(file2)
    pylab.ylabel(file1)
    ...
    # FIX THIS
    #pylab.figure()
    #pylab.hist(np.log2( v1_forloess / v2_forloess), bins=25)
    #if (outplot is not None):
    ...
    #print "B"
    #pylab.hist(np.log2(tmpvals[np.isfinite(tmpvals)]),bins=25)
    #if (outplot is not None):
...
#  print "C"
    ...
    newvals = vals1 - lscalefac*np.maximum(np.zeros_like(predvals),predvals)
    write_grfile(locs1,newvals,outfile)
...
def slope_subtract_counts_log(file1, file2, outfile,offset=0):
    ...
    log_file_1 = np.log2(vals1 + offset)
    log_file_2 = np.log2(vals2 + offset)

    # now I search a bunch of slope/intercept combinations to find the best one
    bestints = []
    fitscores = []
    slopes = np.arange(0,5,0.01)
    #slopes = np.arange(0,5,0.1)
    ...
    for slope in slopes:
        ...
        denom = np.sqrt(slope*slope + 1)

        #print xterm
        #print denom

        for yint in np.arange(-50,50,0.01):
        #for yint in np.arange(-50,50,0.1):
            #print "working on int %f" % yint
            numerators = np.abs( xterm + yint)
            distmed = np.mean( numerators / denom )
            #distsum = np.sum(log_chip_vals - slope * log_ipod_vals + yint)
            #distsum = 0
    ...
    bestind = np.argmax(fitscores)

    bestslope = slopes[bestind]
    bestint = bestints[bestind]
    
    rescaled_f1 = np.power(2, log_file_1 * bestslope + bestint)
...
def lin_subtract_counts_log(file1,file2,outfile,outplot=None,soffset=0.0):
    ...
    xvals_test_plot = np.linspace(xmin,xmax)
    print("done with fit")
    #predvals = np.array( [ get_predval(x) for x in vals2 ] )
    #predvals_forplot = np.array( [ get_predval(x) for x in xvals_test_plot] )
    predvals = np.maximum(slope * vals2 + intercept, np.zeros_like(vals2))
    predvals_forplot = np.maximum(slope * xvals_test_plot + intercept, np.zeros_like(xvals_test_plot))
...
def calc_rzscores(infile,outfile):
        ...
        zscores = (vals - np.median(vals)) / mad
