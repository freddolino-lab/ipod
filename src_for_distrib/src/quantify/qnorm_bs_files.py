...
def calc_qnorm_base( input_arrays ):
    ...
    # first we sort the inputs column-wise and places sorted vals into
    # an array
    ...
    # then pull those sorted arrays together and figure out the median
...
def q_norm_files(hdf_names, ctg_lut, out_dset_name, bs_num):
    ...
    for fname in hdf_names:
        print(fname)
        concat_arr = hdf_utils.concatenate_contig_data(fname)
...
if __name__ == '__main__':
    ...
    for samptype in sample_types:
        ...
        q_norm_files(sample_hdf_fnames, ctg_lut, OUT_DSET, BS_NUM)
