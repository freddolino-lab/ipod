...
parser.add_argument(
    '--config_file',
    help = 'Configuration file for this condition',
    required = True,
    type = str,
)
parser.add_argument(
    '--global_config_file',
    help = 'Top-level configuration file common to all conditions.',
    required = True,
    type = str,
)
...
def do_epod_calls(gr_file_in, outprefix, genomelength):
    """
    Do all epod calling for a given bedgraph file, writing the results to out_prefix
    """
...
myprocs = Pool(conf_dict_global["epods"]["nproc"])
...
for line in conf_str:
    ...
    if args.invert_scores:
        ...
        locs,vals = pu.read_grfile(gr_file_in)
        pu.write_grfile(locs, -1*vals, gr_file_tmp.name)
